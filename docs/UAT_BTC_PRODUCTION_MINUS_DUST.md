...
| P2.4 | Hermes docs + skill with `mcp_servers` snippet         | **PASS** | `docs/HERMES_INTEGRATION.md` has `mcp_servers.readytrader-crypto`; Hermes skill checked out at `/Users/billwilson/Projects/hermes-agent/optional-skills/finance/readytrader-crypto/SKILL.md` + `references/mcp-config.yaml` for this evidence pass. Note: `optional-skills/` is the contribution tree the source repo uses to stage a PR; it is not the runtime install path. The runtime path is `~/.hermes/skills/finance/readytrader-crypto/` (see `docs/HERMES_INTEGRATION.md`). |
