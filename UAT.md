...
`main`, but this repo does **not** yet meet its own graduation gate (Section 6). The
2026-09-24 public-release UAT (`uat/UAT-LOG.md`, run `2026-09-24-01`) ran the dashboard e2e suite
(18/18) and fixed the cross-process approval gap, the Risk Guardian's absence from the order path
and the paper-fill realism defects on its branch; those gates still need a re-verification on the
release SHA, and the owner-only gates remain. Phase 4 (any live capital, including a single
...
**Resolved on 2026-09-24 (UAT branch `uat/2026-09-24-crypto`; re-verify on the release SHA):**

- **Cross-process approval visibility (T5).** `EXECUTION_SESSION_ID`, shared by the MCP server and
  the API server (with the same `EXECUTION_DB_PATH`), puts both in one proposal session; confirming
  is one conditional database update, so a proposal is approved once across processes; a proposal
  executes only in the mode it was made in (`EXEC_313` otherwise).
- **The Risk Guardian was not in the order path.** `validate_trade_risk` refused a 50% bet that
  `place_cex_order` then executed; now every order (paper and live, proposals at creation and at
  approval) passes the Guardian, and live orders are sized against the exchange account.
- **Paper realism and isolation.** Paper orders fill at the market price (limits only when
  marketable), paper swaps at the market rate (they filled at 1.0), and paper mode never reaches an
  exchange account (cancel/replace/read tools answered from the real exchange).
- **Dashboard e2e (T6)** ran 18/18 against the real API harness, with evidence.

**Open blockers:**

- **M4 dashboard e2e is still wired into no GitHub Actions workflow** (T6 ran on the UAT branch;
  it must pass on the release SHA, in CI or with attached evidence).
...
| T5  | Approval gate: `approve_each` vs `auto` (explained)      | `pytest tests/test_approval_api.py -q`; compare `env.example` (`EXECUTION_APPROVAL_MODE=auto`) against `env.live.btc.example` (`EXECUTION_APPROVAL_MODE=approve_each`)                                                                                | `auto` (the paper/dev default) executes immediately; `approve_each` (the mandated BTC-live default) returns a proposal (`request_id`+`confirm_token`) instead — **only in live mode**, paper never proposes. With the MCP and API servers started with the same `EXECUTION_DB_PATH` and `EXECUTION_SESSION_ID`, a proposal made by the MCP process is listed and approvable by the API/dashboard process, exactly once (`docs/ARCHITECTURE.md#approval-gate`) | pytest output; `docs/ARCHITECTURE.md#approval-gate`                                  | **FIXED on the UAT branch 2026-09-24** — cross-process visibility, single-use approval across processes and the mode check are covered by `tests/test_uat_2026_09_24.py` and `tests/test_approval_api.py`; evidence `uat/evidence/2026-09-24-01/BE-08*`. Re-verify on the release SHA                                                                                                                                      |
| T6  | Operator dashboard approve/reject (M4)                   | `cd frontend && npm ci --include=dev && env -u NODE_ENV npm run e2e` (builds, then runs the 18-journey Playwright suite against the real API harness)                                                                                                 | All 18 journeys pass; HTML report generated                                                                                                                                                                                                                                                                                                                                                                                                                   | Playwright HTML report / trace files                                                 | **PASS on the UAT branch 2026-09-24** — 18 passed (`uat/evidence/2026-09-24-01/FE-02.txt`, and the retest after the Risk Guardian changes). `playwright.config.ts` now finds the repo's `.venv` by itself (it defaulted to a path on the original author's machine). Not yet in CI                                                                                                                                         |
...
- [ ] **M4 dashboard e2e wired into CI, or run with attached evidence** on the release SHA — an
  HTML report or recording, not a description of the suite (18/18 on the 2026-09-24 UAT branch:
  `uat/evidence/2026-09-24-01/FE-02*`)
- [ ] **Cross-process approval-proposal visibility fixed or worked around** (see T5) — a
  ...
  fixed on the 2026-09-24 UAT branch (`EXECUTION_SESSION_ID`); tick after re-verification on the
  release SHA
...
| :--------------------------------- | :------------------------------------------ | :--------------------------------------------------- | :----------- | :-------------- |
| PR #9 merged                      | `gh pr view 9`                           | Merged 2026-09-23 - re-verify T10 at UAT          | 2026-09-23 |               |
...
| Dashboard e2e (CI or evidenced)   | `uat/evidence/2026-09-24-01/FE-02*`      | 18/18 on the UAT branch; re-run on release SHA    | 2026-09-24 |               |
| Cross-process approval visibility | `docs/ARCHITECTURE.md#approval-gate`, T5 | Fixed on the UAT branch; re-verify on release SHA | 2026-09-24 |               |
