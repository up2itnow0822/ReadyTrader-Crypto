...
- **PR #9** (closes issue #8: `venue_allowed` denies the documented default
  ...
  later check). **Not yet merged.** Reproduced directly on `main` today: calling
  `execution.router.venue_allowed` with `("auto", "cex")` returns `False`. Blast radius is limited in
